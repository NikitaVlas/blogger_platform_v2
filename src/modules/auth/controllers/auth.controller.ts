...
export class AuthController {
  ...
  async login(req: Request, res: Response<LoginSuccessViewModel>) {
    ...
    const tokens = await this.authSessionService.createTokenPair(
      user._id.toString(),
  ...
  async logout(req: Request, res: Response) {
    ...
    const revoked = await this.authSessionService.logout(refreshToken);
  ...
  async getDevices(req: Request, res: Response) {
    const devices = await this.authSessionService.getActiveDevices(
  ...
  async deleteAllOtherDevices(req: Request, res: Response) {
    const deleted = await this.authSessionService.deleteAllOtherDevices(
      req.cookies?.[REFRESH_TOKEN_COOKIE_NAME],
    );
  ...
  async deleteDevice(req: Request<{ deviceId: string }>, res: Response) {
    const result = await this.authSessionService.deleteDevice(
      req.cookies?.[REFRESH_TOKEN_COOKIE_NAME],
      req.params.deviceId,
    );
  ...
  async registration(
    ...
    if (result.status === "email-send-error") {
  ...
  async registrationConfirmation(
    ...
    const result = await this.registrationService.confirmRegistration(
      req.body.code,
    );
  ...
  async registrationEmailResending(
    ...
    const result = await this.registrationService.resendConfirmationEmail(
      req.body.email,
    );
  ...
  async passwordRecovery(req: Request, res: Response) {
    const recovery = await this.authService.requestPasswordRecovery(
      req.body.email,
    );
    if (recovery) {
      try {
  ...
  async newPassword(req: Request, res: Response) {
    const updated = await this.authService.confirmPasswordRecovery(
